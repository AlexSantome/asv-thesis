...
through conventional parametric sweeps and fabricated on a $220~\text{nm}$ SOI platform. To ensure 
high performance and low loss, the device was implemented using shallow-etched waveguides with a 
$150~\text{nm}$ silicon slab. This choice of partial etching provides a smoother transition for the 
optical modes and reduces scattering compared to fully-etched strip waveguides.

The MMI body features a footprint of approximately $40~\mu\text{m}$ in length and $15~\mu\text{m}$ 
in width. To optimize the mode transition between the access waveguides and the multimode 
interference region, $3~\mu\text{m}$ long tapers were integrated at each port. These access 
structures utilize a $90~\text{nm}$ slab profile, specifically designed to bridge the modal 
mismatch and minimize back-reflections at the junctions.
...
To determine if the performance of the already-optimized $2\times2$ MMI could be further enhanced, 
a \textbf{shape optimization} strategy was implemented using the \texttt{lumopt} framework. The 
core idea was to move beyond the limitations of rectangular boundaries and allow the device to 
evolve into an "organic" profile that better accommodates the internal multimode interference 
patterns.

The optimization utilized the \textbf{gradient-based Adjoint Method}, which calculates the 
sensitivity of a Figure of Merit (FoM) with respect to an arbitrary number of boundary parameters 
using only two full-wave simulations per iteration. The optimization was configured as follows:

\begin{itemize}
    \item \textbf{Initial State:} The starting geometry was the experimentally-validated 
    rectangular MMI.
    \item \textbf{Figure of Merit (FoM):} The goal was to maximize the transmission into the 
    fundamental TE modes of both output ports while simultaneously minimizing the imbalance across 
    the C-band ($1530\text{--}1565~\text{nm}$).
    \item \textbf{Design Parameters:} The side-walls of the MMI body were defined as B-spline 
    curves controlled by multiple points. These points were allowed to shift, effectively 
    "morphing" the MMI width along its longitudinal axis.
    \item \textbf{Fabrication Constraints:} A minimum feature size and curvature radius were 
    enforced to ensure the optimized shape remains fully compatible with the $100~\text{nm}$ 
    lithography rules, preserving the \textbf{fault-tolerant} nature of the fabrication.
\end{itemize}
