...
\begin{table}[!ht]
\centering
\caption{Expected edge coupler performance by simulation at $1550 \text{ nm}$.}
\label{tab:ec_sim_results}
\begin{tabular}{lcccc}
\toprule
\textbf{Optical Fiber} & \textbf{MFD ($\mu\text{m}$)} & \textbf{EC Loss (dB)} & \textbf{PIC In-Out Loss (dB)} & \textbf{Bandwidth} \\ \midrule
SMF-28 & 10.4 & 4.4 & 8.8 & $>60 \text{ nm}$ \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{table}[!ht]
\centering
\caption{Experimental measurement results for different Edge Coupler (EC) instances.}
\label{tab:ec_measurement_results}
\begin{tabular}{lcc}
\toprule
\textbf{Edge Coupler ID} & \textbf{Median @ $1550 \text{ nm}$ (dB)} & \textbf{STD @ $1550 \text{ nm}$ (dB)} \\ \midrule
EC\#1 & 9.10  & 0.22 \\
EC\#2 & 9.56  & 0.11 \\
EC\#3 & 10.59 & 0.42 \\
EC\#4 & 11.20 & 0.24 \\
