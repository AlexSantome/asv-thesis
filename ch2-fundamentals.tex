...
\cite{khaniSiPMLHighbandwidthOptical2021} for data centers, high-speed optical transceivers, 
microwave photonics, sensing, and emerging computing paradigms. The platform supports both passive 
and active functionalities. While silicon itself lacks a strong second-order electro-optic effect, 
efficient modulation can be achieved through carrier-based mechanisms such as plasma dispersion, 
and photodetection can be implemented via heterogeneous integration with germanium. Furthermore, 
hybrid and heterogeneous integration with III-V materials enables on-chip light sources, addressing 
silicon's indirect bandgap limitation.
...
in densely integrated circuits \cite{wattsAdiabaticThermoopticMach2013,
jacquesOptimizationThermoopticPhaseshifter2019, coenenAnalysisThermalCrosstalk2022}.
...
\noindent where $\kappa$ is the coupling coefficient, determined by the modal overlap and 
separation distance. 
\newpage
\noindent The characteristic length required for complete power transfer, known as the coupling 
length, is
...
\noindent where $\beta_0$ and $\beta_1$ are the propagation constants of the fundamental and 
first-order modes, respectively. For an MMI operating in the restricted paired interference regime 
(where the input field is launched at $x = \pm W/6$), the length required to form a two-fold 
self-image (3-dB splitter) is significantly reduced and can be approximated as
...
\noindent where $\lambda$ is the operating wavelength, $n_{\text{eff}}$ is the effective index of 
the guided mode, and $n_{\text{clad}}$ is the refractive index of the cladding material.
...
fiber. Figure \ref{fig:ch2-edge_coupler_info}(c) shows the impact of lateral misalignment on the 
total coupling loss. In the context of large-scale programmable meshes, these losses contribute to 
the overall power budget and can be a source of systematic error if the packaging process exhibits 
...
\ref{fig:ch2-tops}. As shown in the cross-section in (a), the heat generated by the metallic 
element must diffuse through the upper cladding to reach the waveguide core. The simulated 
temperature gradient in (b) demonstrates that while the silicon core reaches the necessary 
$\Delta T$ for phase shifting, a significant amount of heat also spreads laterally.
