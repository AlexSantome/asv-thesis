...
The performance of directional couplers is sensitive to wavelength variations and fabrication 
tolerances, as small deviations in waveguide width or gap separation modify the effective coupling 
coefficient. These sensitivities must be carefully managed in large-scale programmable 
architectures.
...
\begin{figure}[h]
    ...
    (a) Top-view.
    (b) Cross-section.}
    \label{fig:ch2-directional_coupler}
\end{figure}

% \textbf{Suggested figure:} Schematic of two parallel waveguides with coupling region length $L$, 
% including sinusoidal power exchange plot.
...
MMIs are widely used to implement robust 1×2 and 2×2 splitters. Compared to directional couplers, 
they exhibit reduced sensitivity to fabrication variations and wavelength fluctuations, at the 
expense of larger footprint. Their tolerance to dimensional deviations makes them attractive for 
large-scale programmable meshes.

\textbf{Suggested figure:} Multimode section showing input excitation and formation of two output 
images at the output plane.
