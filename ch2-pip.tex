...
As shown in Figure \ref{fig:ch2-edge_coupler_info}, this is often achieved by narrowing the silicon 
waveguide width (inverted taper), which forces the optical field to expand into the cladding, 
effectively increasing the spot size before it reaches the chip facet.
...
The physical principle behind this expansion is shown in the mode field profiles of Figure 
\ref{fig:ch2-edge_coupler_info}b. By reducing the core width, the optical field is forced out of the 
silicon into the surrounding cladding, effectively increasing the spot size. Unlike grating 
couplers, edge couplers are non-resonant, providing a significantly broader operational bandwidth 
and lower insertion losses (typically $< 1$ dB per facet).
...
interferometric units capable of realizing arbitrary $2 \times 2$ transformations. The integration 
of these passive and active elements culminates in the definition of the \textit{Programmable 
Unit Cell} (PUC), the fundamental building block of the mesh, which will be introduced in detail 
after discussing the physical mechanisms enabling dynamic phase control in the following section.
...
spread leads to \textbf{thermal crosstalk}, where the operation of one heater inadvertently affects 
...
discussed in Section \ref{sec:passive_bb} culminates in the definition of the \textit{Programmable Unit Cell} (PUC). The PUC acts as the fundamental building block of the mesh, enabling the synthesis of arbitrary $2 \times 2$ unitary transformations through the coordinated control of phase and coupling.
...
The transition from individual building blocks to a functional programmable processor is realized 
through the definition of the \textit{Programmable Unit Cell} (PUC). The PUC is the fundamental, 
repeating element that, when interconnected in a mesh-like topology, enables the synthesis of 
complex, large-scale optical functions. In the context of this thesis, the PUC is treated as a 
reconfigurable $2 \times 2$ port device capable of performing arbitrary unitary transformations.

\subsection{Architecture and Principle of Operation}

The most prevalent architecture for a PUC is based on the \textit{Mach-Zehnder Interferometer} 
(MZI). As illustrated in Figure \ref{fig:puc_mzi_architecture}, a standard PUC consists of two 
$3$-dB couplers (either Directional Couplers or MMIs) connected by two waveguide arms, where at 
least one of them incorporates an active phase shifter to induce a differential phase shift 
$\theta$.

\begin{figure}[htbp]
    \centering
    % \includegraphics[width=0.8\textwidth]{figures/puc_mzi_architecture}
    \caption{Architecture of a Mach-Zehnder Interferometer (MZI) based programmable unit cell. The 
    internal phase shifter $\theta$ controls the power splitting ratio at the outputs, while an 
    optional external phase shifter $\phi$ controls the relative phase.}
    \label{fig:puc_mzi_architecture}
\end{figure}



The operation of the PUC can be described by the multiplication of the transfer matrices of its 
constituent parts. For a balanced MZI with two ideal $50:50$ couplers and a differential phase 
shift $\theta$ between the arms, the resulting transfer matrix $\mathbf{M}_{\text{MZI}}$ is derived 
as:

\begin{equation}
\mathbf{M}_{\text{MZI}} = \mathbf{M}_{\text{coupler}} \cdot \mathbf{M}_{\text{phase}}(\theta) 
\cdot \mathbf{M}_{\text{coupler}}
\label{eq:mzi_matrix_derivation}
\end{equation}

Assuming symmetric $3$-dB couplers with a transfer matrix as defined in Section 2.3.1, the total 
matrix of the PUC can be expressed as:

\begin{equation}
\mathbf{M}_{\text{MZI}}(\theta) = i e^{j\frac{\theta}{2}} 
\begin{pmatrix} 
\sin\left(\frac{\theta}{2}\right) & \cos\left(\frac{\theta}{2}\right) \\[10pt] 
\cos\left(\frac{\theta}{2}\right) & -\sin\left(\frac{\theta}{2}\right) 
\end{pmatrix}
\label{eq:mzi_matrix_final}
\end{equation}

This mathematical framework shows that by tuning the internal phase $\theta$, the PUC transitions 
between two fundamental routing states. It should be noted that the specific mapping of $\theta$ to 
these states depends on the phase relationship of the $3$-dB couplers; assuming standard symmetric 
couplers with a $\pi/2$ phase shift between their cross and bar ports, the states are defined as:
\begin{itemize}
    \item \textbf{Cross State ($\theta = 0$):} Light entering port 1 exits through port 2, and vice 
    versa. This occurs when the two $3$-dB coupling stages and the zero differential phase result 
    in purely constructive interference at the cross port.
    \item \textbf{Bar State ($\theta = \pi$):} Light entering port 1 exits through port 1. In this 
    condition, the total phase accumulated through the MZI arms causes the signal to exit through 
    the same port it entered.
\end{itemize}



In a fully reconfigurable mesh, a second phase shifter $\phi$ is often added at one of the output 
ports. While $\theta$ controls the power distribution (the "coupling" of the unit), $\phi$ controls 
the \textbf{relative phase} between the two outputs. This combination allows the PUC to implement 
any arbitrary transformation in the $U(2)$ group, providing the universal tunability required for 
photonic computing and signal processing.


\subsection{Degrees of Freedom and Universal Control}

A general $2 \times 2$ unitary transformation $U(2)$ can be described by four degrees of freedom. 
However, in most programmable photonic applications, the focus lies on the relative power 
distribution and the relative phase between the ports, which reduces the required control parameters 
to two primary variables: the \textit{internal} phase shift ($\theta$) and the \textit{external} 
phase shift ($\phi$).

The internal phase shifter $\theta$, located within the MZI arms, functions as the 
\textbf{amplitude control}. By modulating $\theta$, the interference condition at the output 
coupler is modified, allowing the power splitting ratio to be continuously tuned. In the Bloch 
sphere representation (see Figure \ref{fig:puc_degrees}), this operation corresponds to a rotation 
that changes the "latitude" of the state, effectively moving the signal between the Bar and Cross 
states.

To achieve a truly universal $2 \times 2$ transformation, an external phase shifter $\phi$ is added 
at one of the output ports. This element provides \textbf{phase control}, allowing the adjustment 
of the relative phase between the two output modes without altering their power levels. The total 
transfer matrix of such a universal PUC is given by:

\begin{equation}
\mathbf{M}_{\text{PUC}}(\theta, \phi) = 
\begin{pmatrix} 1 & 0 \\ 0 & e^{j\phi} \end{pmatrix} 
\begin{pmatrix} 
\sin\left(\frac{\theta}{2}\right) & \cos\left(\frac{\theta}{2}\right) \\[10pt] 
\cos\left(\frac{\theta}{2}\right) & -\sin\left(\frac{\theta}{2}\right) 
\end{pmatrix}
\label{eq:universal_puc}
\end{equation}

This two-degree-of-freedom architecture is essential for universal photonic computing. However, its 
performance is highly sensitive to the balance of the constituent couplers. If the $3$-dB couplers 
exhibit deviations from their nominal $50:50$ splitting ratio due to fabrication tolerances, the 
PUC may no longer be able to reach the ideal Bar or Cross states, leading to a reduced extinction 
ratio and limiting the overall fidelity of the mesh. These limitations and their impact on fault 
tolerance are discussed in detail in Section 2.6.
